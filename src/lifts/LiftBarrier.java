...
/**
 * Handles events in a lift.
 * Keeps track of people waiting for the elevator and releases them when 
 * doors open and one of the conditions are met (in floor requested OR lift broken)
 * This is only used for people to get OUTSIDE the elevator.
...
public class LiftBarrier {
    ...
    public synchronized void arrive(int stop){
        ...
        System.out.println("Person is now waiting for stop.");
        if((event && floorReached == stop) || (event && broken)){
            return;
        }
        while(!event && (floorReached == stop || broken)){//leave when they get to stop OR lift broken
            try{
    ...
    public synchronized void raiseBroken(){
        ...
        broken = true;
        notifyAll();
        while(peopleWaiting.size() != 0){//will wait for everyone to leave
            try{
                this.wait();
            }catch (InterruptedException ie){
    ...
    /**
     * Get remaining people who want to get out in that floor.
     * Internal use only
    ...
    private int peopleGettingOffThisFloor(){
        int result = 0;
        for(int i = 0; i < 7; i++){
            if(peopleWaiting.get(i) == floorReached) result++;
        }
        return result;
    }
}
